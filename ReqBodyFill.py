...
#术语: goal_rpUrl == 目标仓库的url, goal_rpOrg == 目标仓库的组织, goal_rpName == 目标仓库的名字, goal_rpDesc == 目标仓库的描述， goal_rpIsPub == 目标仓库是否开源, goal_rpLang == 目标仓库语言
...
def ReqBodyFillF(goalRp:GoalRpT):
    ...
    goal_rpUrl:str=goalRp.goal_rpUrl
    ...
    tmpl_HPRBUD_fP:str="template__HttpPostReqBdyUrlEncoded.txt"
    goal_tmpl_HPRBUD_fP=f"{tmpl_HPRBUD_fP.replace('template__','')}__{goal_rpName}"

    reqBdyUrlEncoded:str=Path(tmpl_HPRBUD_fP).read_text()#f_1
    ...
    #填写 目的仓库 描述
    fldDict["project[import_url]"]=goal_rpUrl
    ...
    fldDict["language"]=goal_rpLang

    goal_fldLs:typing.List[str]=[_concatKeyValBySep(keyJ,valJ,kSEPv) for keyJ,valJ in fldDict.items() ]#逆f2
    goal_reqBdy:str=fldSepOnUrl.join(goal_fldLs)#逆f1
    ...
    Path(f"{outHPRBUD_dir}/{goal_rpName}").write_text(goal_reqBdyUrlEncoded)#逆f_1
...
if __name__ == "__main__":
    ...
    goal_rpName:str="pytorch--QNNPACK"
    ReqBodyFillF(
    goal_rpUrl="https://github.com/pytorch/QNNPACK.git"
    ,goal_rpOrg="mirrr"
