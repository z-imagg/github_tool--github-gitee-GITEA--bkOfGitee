...
def main_cmd():
    task1 = richPrgrs.add_task("[red]Downloading...", total=200)
    task2 = richPrgrs.add_task("[green]Processing...", total=200)
    task3 = richPrgrs.add_task("[cyan]Cooking...", total=200)

    while not richPrgrs.finished:
        richPrgrs.update(task1, advance=0.5)
        richPrgrs.update(task2, advance=0.3)
        richPrgrs.update(task3, advance=0.9)
        time.sleep(0.02)
